...
module.exports = function processGithubEmbed(block) {
  ...
  const { github = {}, ...urlConfig } =
    urls.find(({ regex }) => regex && url.match(regex)) || {};
  ...
  return checkCache(url, options, () =>
    Promise.resolve(extractSnippet(url, options)),
  );
};
...
function extractSnippet(url, options) {
  ...
  return Promise.try(() => github.repos.getContents(request))
    ...
    .then((code) => {
      ...
      return `<pre><code class="${language}">${entityEncoder.htmlEncode(
        trimmed,
      )}</code></pre>${link}`;
    });
}
