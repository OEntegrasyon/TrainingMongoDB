...
function processSEO(page) {
	var seo = this.config.get('pluginsConfig')['myseo2']
			...
			if (/\.html$/.test(file)) {
				...
				if(match5){
					...
					if(match6){
						...
						let rule = /<title>(.*?)<\/title>/g;
						originalTitle = rule.exec(match5[0])[1];
						newTitle = originalTitle.replace('· GitBook', seo.titleAfterfix)
						data = data.replace(match5[0], '<title>' + newTitle + '</title>')
					}
				}
				// nofollow
